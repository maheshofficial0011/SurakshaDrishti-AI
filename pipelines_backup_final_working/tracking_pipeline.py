"""
SurakshaDrishti AI — Final MVP Tracking Pipeline
...
Overlay Fix:
- Runtime title/stats are shown on the top-left.
- LIVE and CAM-01 badges are shown on the top-right.
- This prevents badge/title overlap in the camera preview and dashboard stream.
"""
...
def _fit_text_scale(text, font, max_width, start_scale, thickness, min_scale=0.42):
    """
    Shrink OpenCV text scale until it fits inside max_width.

    This protects the overlay from overflowing on smaller frames.
    """
...
def _draw_hud_badge(
    ...
    """
    Draw one top-right HUD badge.

    Args:
        right_x:
            Right edge of the badge. This makes it easy to align badges
            from right to left.

    Returns:
        Width of the drawn badge.
    """
    ...
    x1 = int(right_x - badge_w)
    y1 = int(top_y)
    x2 = int(right_x)
    y2 = int(top_y + badge_h)
...
def draw_runtime_overlay(
    frame, frame_count, tracked_count, detection_count=0, person_count=0
):
    """
    Draw clean runtime camera overlay.

    Fixed layout:
    - Top-left:
        SurakshaDrishti AI | Final MVP
        Camera: CAM-01 | Tracked: N | Frame: N

    - Top-right:
        [LIVE] [CAM-01]

    This fixes the previous overlap where LIVE/CAM badges were drawn
    ...
    title_text = "SurakshaDrishti AI | Final MVP"
    stats_text = (
        f"Camera: {CAMERA_ID} | Det: {detection_count} | "
        f"Person: {person_count} | Tracked: {tracked_count} | Frame: {frame_count}"
    )
    ...
    white = (255, 255, 255)
    soft_blue_white = (180, 220, 255)
    live_green = (0, 255, 120)
    cam_orange = (255, 180, 0)
    ...
    # Reserve top-right area for LIVE and CAM badges.
    right_badge_reserved_width = 190

    max_panel_width = max(
        260,
        width - panel_x - right_badge_reserved_width - 24,
    )
    ...
    cam_right = width - right_margin

    cam_badge_width = _draw_hud_badge(
        frame=frame,
        text=CAMERA_ID,
        right_x=cam_right,
        top_y=top_margin,
        border_color=cam_orange,
        text_color=white,
        font=font,
        font_scale=0.52,
        thickness=2,
...
def main():
    ...
    print_banner()
    print_info("Starting SurakshaDrishti AI pipeline...")
    ...
    try:
        ...
        window_name = "SurakshaDrishti AI - Final MVP"
        ...
        while True:
            ...
            # --------------------------------------------------
            # Draw order matters:
            # 1. Detection/tracking boxes first.
            # 2. Runtime overlay last so HUD remains visible.
