"""
SurakshaDrishti AI — SQLite Database Layer
...
- Support authority response incident workflow.
...
Authority Workflow:
- New authority records start as PENDING.
- Operator can move them through:
  PENDING -> ASSIGNED -> DISPATCHED/RUNNING -> RESOLVED

Note:
- The database stores DISPATCHED.
- The frontend can display DISPATCHED as "RUNNING".
"""
...
def get_connection():
    ...
    Kept simple so existing backend routes can continue using it safely.
    """
...
def init_db():
    ...
    # -----------------------------------------------------------------
    # Authority dispatch / incident table
    # -----------------------------------------------------------------
    # This table stores authority incidents connected to alerts/SOS.
    cursor.execute("""
    ...
    # -----------------------------------------------------------------
    # Safe dispatch workflow migration
    # -----------------------------------------------------------------
...
def save_event(event: dict):
    ...
    if not location_label:
        location_label = event.get("incident_location") or demo_location["label"]
...
# ---------------------------------------------------------------------
# Dispatch / Authority Incident support
# ---------------------------------------------------------------------


def save_dispatch(dispatch: dict):
    """
    Save one authority dispatch / incident record.

    Workflow:
    - New records normally start as PENDING.
    - Operator can later update:
      PENDING -> ASSIGNED -> DISPATCHED/RUNNING -> RESOLVED
...
def get_dispatches(limit: int = 50):
    ...
    Includes workflow timestamps so the dashboard can show:
    - pending
    - assigned
    - running/dispatched
    - resolved
...
def get_dispatch_by_dispatch_id(dispatch_id: str):
    """
    Return one dispatch / authority incident by public dispatch_id.
    """
...
def update_dispatch_status(
    ...
    """
    Update one authority dispatch / incident status.
    ...
    UI can display DISPATCHED as "RUNNING".

    Args:
        dispatch_id: public dispatch id, e.g. DSP-0001
        status: new workflow status
        unit_name: optional updated unit name
        unit_type: optional updated unit type
        eta_minutes: optional ETA
        resolution_note: optional close note

    Returns:
        dict or None: updated dispatch record
    """

    allowed_statuses = {"PENDING", "ASSIGNED", "DISPATCHED", "RESOLVED"}
    ...
    if status == "RESOLVED":
        ...
        params.append(resolution_note or "Incident resolved by operator.")
    ...
    return get_dispatch_by_dispatch_id(dispatch_id)


def get_dispatch_status_counts():
    """
    Return dispatch workflow counts for Authority Response Center.
